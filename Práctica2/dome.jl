...
inputs = Matrix(Float64.(df[:, 1:63]))
targets = Vector(df[:, :letra])
clases_presentes = sort(unique(targets))

# valores para número de nodos
valores_nodos = [10, 20, 30, 50, 75, 100, 125, 150]
...
println("\n" * "="^65)
println("RESULTADOS EXPERIMENTALES: DoME")
println("="^65)
@printf("%-20s %-20s %-20s\n", "maximumNodes", "Accuracy (Media)", "Desv. Típica")
println("-"^65)

for nodos in valores_nodos
    # hiperparámetros
    params = Dict("maximumNodes" => nodos)
    
    # modelCrossValidation devuelve una tupla con métricas y la matriz
    resultados = modelCrossValidation(:DoME, params, (inputs, targets), indices_cv)
    
    acc_media, acc_std = resultados[1]
    matriz_confusion = resultados[8] # la matriz está en la posición 8
    
    @printf("%-20d %-20.4f %-20.4f\n", nodos, acc_media, acc_std)
